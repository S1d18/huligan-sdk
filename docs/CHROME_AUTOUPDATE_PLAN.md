...
## Phase 4 — автоматизация ПУБЛИКАЦИИ манифеста (не сборки) — DONE (2026-07-15)

Сборку GHA не потянет (локальная Windows-машина 192.168.1.120, ~100 ГБ исходников,
часы ninja). Автоматизировали только метаданные — всё в репо **huligan-releases**:

- [x] `tools/publish.py`: считает sha256/size из локального zip, дописывает запись в
  `versions`; `latest` двигается только с `--set-latest` (невалидированный билд не
  промоутится); `min_conf_schema` по умолчанию переносится (bump `--min-conf-schema`);
  `--dry-run`, `--commit`. Отказывается писать невалидный манифест.
- [x] `tools/validate_manifest.py`: чистая offline-проверка схемы (tag/asset/sha256/
  min_conf_schema/latest∈versions) — переиспользуется и скриптом, и GHA.
- [x] `.github/workflows/validate-manifest.yml`: на PR/push, трогающих манифест или
  tools, гоняет валидатор + `tools/test_manifest_tools.py`.
- [x] `tools/README.md` — процесс публикации; `tools/test_manifest_tools.py` (16 тестов).
- Убрано ручное редактирование JSON и рассинхрон sha256 между манифестом и релизом.

> Проверку «asset реально существует в GitHub Release» намеренно НЕ включили в GHA
> по умолчанию (нужна сеть/токен, недетерминизм) — валидатор чисто структурный.
