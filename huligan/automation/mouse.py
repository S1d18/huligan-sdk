...
MICRO_STEP_DELAY_RANGE = (0.002, 0.008)
SMOOTH_MOVE_DELAY = (0.002, 0.008)

# --- Элементы "очеловечивания" движения мыши ---
SHIFT_OFFSET_BEFORE_CLICK_COUNT = (1, 3)
SHIFT_OFFSET_BEFORE_CLICK_RANGE = (-2, 2)
JITTER_AMOUNT = 3
ENABLE_SUB_STEP_CURVING = True
SUB_STEP_CURVE_AMOUNT_MAX = 3
SUB_STEP_SEGMENTS = 2

# --- Параметры скролла ---
SCROLL_CHUNK_SIZE_RANGE = (100, 250)
SCROLL_CHUNK_DELAY_RANGE = (0.02, 0.07)
SCROLL_LONG_PAUSE_PROBABILITY = 0.15
SCROLL_LONG_PAUSE_DURATION_RANGE = (0.4, 1.2)
SCROLL_MOUSE_WIGGLE_DURING_PAUSE = True
SCROLL_MOUSE_WIGGLE_AMOUNT = (5, 15)
SCROLL_PAUSE_AFTER_CHUNKS = (0.06, 0.15)
SCROLL_JITTER_PER_CHUNK = (-20, 20)
REVERSE_SCROLL_PROBABILITY = 0.10

# --- Функции плавности Pytweening ---
EASING_FUNCTIONS_ALL = [
...
CLICK_PRESS_DURATION_RANGE = (0.05, 0.15)
ENABLE_POST_CLICK_MOVE = True
POST_CLICK_MOVE_DISTANCE_RANGE = (25, 50)
POST_CLICK_MOVE_DURATION_RANGE = (0.6, 1.2)
POST_CLICK_MOVE_STEPS_RANGE = (5, 10)
POST_CLICK_EASING_FUNCTIONS = [pt.easeOutSine, pt.easeOutQuad, pt.easeOutCubic, pt.easeOutExpo, pt.easeOutBack]
...
def get_random_point_in_ellipse(width: float, height: float):
    """Утилита — не зависит от async/sync"""
    cx = width / 2
    cy = height / 2
    rx = width / 2
    ry = height / 2
...
if MODE == "async":
    from .cdp_helpers import (
        cdp_bounding_box_locator, cdp_wait_for_visible,
        cdp_viewport_size, cdp_scroll_y, cdp_document_height,
    )
    ...
    async def get_viewport_size(page):
        ...
        if vp:
            return vp
        # Fallback to CDP (works via connect_over_cdp)
        return await cdp_viewport_size(page)

    async def _do_tweened_move(page, start_x, start_y, end_x, end_y, duration_s, steps, ease_func):
        mouse = page.mouse
        current_x, current_y = start_x, start_y
        distance = math.dist((start_x, start_y), (end_x, end_y))
        if distance < 0.1:
            if round(start_x) != round(end_x) or round(start_y) != round(end_y):
                await mouse.move(round(end_x), round(end_y))
                if DRAW_MODE:
                    await mouse.down()
            return end_x, end_y
    ...
    async def human_like_mouse_click(locator: Locator, time_sleep: float = TIME_SLEEP_DEFAULT, speed_mode: str = "fast",
                                     no_scroll: bool = False):
        """
        Человекоподобный клик с траекторией мыши (ASYNC режим).

        Args:
            locator: Patchright/Playwright Locator
            time_sleep: Базовая задержка после клика (сек)
            speed_mode: "fast"/"medium"/"slow"/"manual"
            no_scroll: Отключить скролл к элементу

        Returns:
            (x, y) позиция после клика или None при ошибке
        """
        page = locator.page
        ...
        try:
            visible = await cdp_wait_for_visible(locator, timeout=10.0)
            if not visible:
                logger.error(f"[human_like_mouse_click] Locator not visible after 10s")
                return
        except Exception as e:
            logger.error(f"[human_like_mouse_click] Locator did not appear in time: {e}")
            return

        # Track scroll position via CDP
        _scroll_y = await cdp_scroll_y(page)
        initial_scroll_y = _scroll_y

        async def scroll_to_element_humanly():
            nonlocal page, locator, _scroll_y
            logger.debug(f"[ASYNC] HumanScroll: Initiating for {locator}")
            vp = await get_viewport_size(page)
            ...
            for attempt in range(max_attempts):
                element_info = None
                try:
                    element_info = await cdp_bounding_box_locator(locator, timeout=0.5)
                ...
                if element_info:
                    ...
                    if el_y < viewport_height and el_y + el_h > 0:
                        ...
                        if el_h > 0:
                            vis_ratio = (vis_bot - vis_top) / el_h
                    if vis_ratio > 0.95:
                ...
                current_sy = await cdp_scroll_y(page)
                _scroll_y = current_sy
                doc_sh = await cdp_document_height(page)
                ...
                if (at_bottom and scroll_direction > 0) or (at_top and scroll_direction < 0):
                    final_check = None
                    try:
                        final_check = await cdp_bounding_box_locator(locator, timeout=0.3)
                    except:
                        pass
                    if final_check and 0 <= final_check["y"] < viewport_height:
                        return True
                    return False
                ...
                while abs(target_scroll_delta) > 20 and num_chunks < 5:
                    ...
                    if abs(chunk_size) < 10:
                        break

                    logger.trace(f"HumanScroll: Chunk scroll by {chunk_size:.0f}px")
                    await page.mouse.wheel(0, round(chunk_size))
                    _scroll_y += chunk_size
                    target_scroll_delta -= chunk_size
                    current_sy += chunk_size
                    num_chunks += 1
                    await asyncio.sleep(random.uniform(*SCROLL_CHUNK_DELAY_RANGE))

                    if SCROLL_MOUSE_WIGGLE_DURING_PAUSE and random.random() < 0.3:
                        vp_s_wiggle = await get_viewport_size(page)
                        if vp_s_wiggle:
                            ...
                            await _do_tweened_move(page, cx_wiggle, cy_wiggle, wiggle_x, wiggle_y, 0.1, 2,
                                                    pt.easeOutSine)

                if num_chunks == 0 and abs(scroll_direction) > 0:
                    emergency_delta = round(min(viewport_height * 0.1, 50) * scroll_direction)
                    await page.mouse.wheel(0, emergency_delta)
                    _scroll_y += emergency_delta
                    await asyncio.sleep(random.uniform(*SCROLL_PAUSE_AFTER_CHUNKS))

                if random.random() < SCROLL_LONG_PAUSE_PROBABILITY:
                    await asyncio.sleep(random.uniform(*SCROLL_LONG_PAUSE_DURATION_RANGE))
                elif num_chunks > 0:
                    await asyncio.sleep(random.uniform(*SCROLL_PAUSE_AFTER_CHUNKS))
        ...
        try:
            if not await scroll_to_element_humanly():
                logger.error(f"Element {locator} not visible after scroll/check. Aborting.")
                return None

            if abs(_scroll_y - initial_scroll_y) > 10:
                _human_click_last_pos = None
            ...
            for attempt in range(MAX_ATTEMPTS_BEFORE_CLICK):
                try:
                    box = await cdp_bounding_box_locator(locator, timeout=1.0)
                    vp_s = await get_viewport_size(page)
                    if box and vp_s:
            ...
            logger.info(
                f"[ASYNC] Clicking {locator}. Box:({box['x']:.0f},{box['y']:.0f} w:{box['width']:.0f},h:{box['height']:.0f}) Mode:{speed_mode}")
            mouse = page.mouse
            ...
            if _human_click_last_pos:
                ...
                await mouse.move(round(start_x), round(start_y))
                if DRAW_MODE:
                    await mouse.down()
            ...
            for i in range(steps):
                ...
                if p_enable_sub_step_curving and math.dist((current_x, current_y), (
                    ...
                    for k_sub in range(1, p_sub_step_segments + 1):
                        ...
                        await mouse.move(round(cvd_x), round(cvd_y))
                        if DRAW_MODE:
                            await mouse.down()
                    ...
                    await mouse.move(round(next_tgt_x), round(next_tgt_y))
                    if DRAW_MODE:
                        await mouse.down()
                    current_x, current_y = next_tgt_x, next_tgt_y

                if i < steps - 1 and p_micro_step_delay_range[1] > 0:
                    await asyncio.sleep(random.uniform(*p_micro_step_delay_range))
            ...
            if distance > 10 and steps > 1 and p_smooth_move_delay[1] > 0:
                for j_sm in range(2):
                    ...
                    await mouse.move(round(fin_x), round(fin_y))
                    if DRAW_MODE:
                        await mouse.down()
                    if j_sm < 1:
                        await asyncio.sleep(random.uniform(*p_smooth_move_delay))
            ...
            if num_pre_click_shifts > 0 and (
                    p_shift_offset_before_click_range[1] != p_shift_offset_before_click_range[0]):
                ...
                for _ in range(num_pre_click_shifts):
                    ...
                    if round(sh_x) != round(current_x) or round(sh_y) != round(current_y):
                        await _do_tweened_move(page, current_x, current_y, sh_x, sh_y, random.uniform(0.01, 0.03),
            ...
            await mouse.down()
            await asyncio.sleep(random.uniform(*p_click_press_duration_range))
            await mouse.up()
            if DRAW_MODE:
                await mouse.down()
            ...
            await asyncio.sleep(random.uniform(time_sleep, time_sleep + 1.0))
            ...
            if p_enable_post_click_move:
                ...
                fin_pos_x, fin_pos_y = await _do_tweened_move(page, current_x, current_y, post_tgt_x, post_tgt_y,
                                                               post_dur,
            ...
            try:
                await page.mouse.up()
                await asyncio.sleep(random.uniform(time_sleep, time_sleep + 1))
            except Exception:
                pass
            _human_click_last_pos = None
            return None
...
# ==================== SYNC РЕЖИМ ====================
    ...
    def get_viewport_size(page):
        vp = page.viewport_size
        if not vp:
            logger.warning("page.viewport_size is None, using default 1280x720")
    ...
    def human_like_mouse_click(locator, time_sleep: float = TIME_SLEEP_DEFAULT, speed_mode: str = "fast",
                               no_scroll: bool = False):
        """
        Человекоподобный клик с траекторией мыши (SYNC режим).
        ...
        # Track scroll position without page.evaluate (Huligan Chrome blocks it)
        _scroll_y = 0
        initial_scroll_y = 0

        def scroll_to_element_humanly():
            nonlocal page, locator, _scroll_y
            logger.debug(f"[SYNC] HumanScroll: Initiating for {locator}")
            vp = get_viewport_size(page)
            viewport_height = vp["height"]
            if not viewport_height:
                return False
            max_attempts = 25
            ...
            for attempt in range(max_attempts):
                ...
                current_sy = _scroll_y
                # Estimate document height: use large default (scroll stops when element visible)
                doc_sh = max(viewport_height * 20, viewport_height)
                at_bottom = scroll_direction > 0 and current_sy + viewport_height >= doc_sh - 5
                at_top = scroll_direction < 0 and current_sy <= 5
                ...
                if num_chunks == 0 and abs(scroll_direction) > 0:
                    emergency_delta = round(min(viewport_height * 0.1, 50) * scroll_direction)
                    page.mouse.wheel(0, emergency_delta)
                    _scroll_y += emergency_delta
        ...
        try:
            ...
            logger.info(
                f"[SYNC] Clicking {locator}. Box:({box['x']:.0f},{box['y']:.0f} w:{box['width']:.0f},h:{box['height']:.0f}) Mode:{speed_mode}")
            mouse = page.mouse
            width, height = max(box["width"], 1), max(box["height"], 1)
            rand_x_el, rand_y_el = get_random_point_in_ellipse(width, height)
            end_x, end_y = box["x"] + rand_x_el, box["y"] + rand_y_el
            ...
            if distance > 10 and steps > 1 and p_smooth_move_delay[1] > 0:
                for j_sm in range(2):
                    ...
                    if DRAW_MODE:
                        mouse.down()
                    if j_sm < 1:
                        time.sleep(random.uniform(*p_smooth_move_delay))
                    current_x, current_y = fin_x, fin_y
            elif abs(current_x - end_x) > 0.5 or abs(current_y - end_y) > 0.5:
                mouse.move(round(end_x), round(end_y))
                if DRAW_MODE:
